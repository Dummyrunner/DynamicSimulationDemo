...
class SubmarinePlant(PlantBase):
    ...
    def step(self, time_delta):
        ...
        # print(
        #     f"thrust: {thrust}; bound: {input_bound}; saturated_thrust: {saturated_thrust}"
        # )
...
class Game:
    ...
    def main_loop(self):
        ...
        while running:
            ...
            if self.game_state == GameState.RUNNING:
                self.plant.state = SubmarineState(
                    depth=self.plant.submarine.body.position.y,
                    vertical_velocity=self.plant.submarine.body.velocity.y,
                )
                input_from_key = (
                ...
                current_system_state = self.plant.get_state()
                position_error = current_system_state.depth - self.reference_signal
                current_system_state_error = SubmarineState(
                    depth=position_error,
                    vertical_velocity=current_system_state.vertical_velocity - 0,
                )
                input_from_controller = (
                    self.controller.get_control_input(current_system_state_error)
                    if self.control_active
                    else 0.0
                )
                ...
                # print(f"control input: {input_from_controller}")
...
if __name__ == "__main__":
    ...
    print(f"CONTROLLER GAIN MATRIX: {k_controller}")
